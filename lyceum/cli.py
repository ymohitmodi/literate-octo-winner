"""Lyceum command-line interface — the conductor of the whole lifecycle.

    python -m lyceum.cli data       --preset tiny     # corpus + curation + tokenizer
    python -m lyceum.cli pretrain   --preset tiny     # self-supervised pretraining
    python -m lyceum.cli align      --preset tiny     # SFT then DPO
    python -m lyceum.cli eval       --preset tiny     # capability + red-team + ship gate
    python -m lyceum.cli security   --preset tiny     # attack/defense demonstrations
    python -m lyceum.cli chat       --preset tiny     # interactive generation
    python -m lyceum.cli agent      --preset tiny     # ReAct agent + gateway demo
    python -m lyceum.cli serve      --preset tiny     # FastAPI inference server
    python -m lyceum.cli all        --preset nano     # the entire pipeline, end to end
    python -m lyceum.cli doctor                       # detect GPU/CPU + enabled features
    python -m lyceum.cli grpo       --preset tiny     # RLVR reasoning (GRPO, verifiable reward)
    python -m lyceum.cli interpret  --preset tiny     # sparse autoencoder + activation steering
    python -m lyceum.cli scaling    --preset tiny     # train a model ladder, fit a scaling law
...
def cmd_chat(args):
    ...
    while True:
        ...
        if not p:
            break
        print("lyceum>", eng.generate(p, max_new_tokens=args.max_new_tokens,
...
def cmd_doctor(args):
    """Report detected hardware and which advanced features are enabled."""
    from . import hardware
    print(hardware.report())
    print(f"  (use --preset auto to size the pipeline to this machine; "
          f"recommended = {hardware.recommend_preset()})")


def cmd_grpo(args):
    """RLVR reasoning with GRPO on a verifiable arithmetic task."""
    from .data.tokenizer import BPETokenizer
    from .model.transformer import LyceumLM
    from .train.checkpoint import load_checkpoint
    ...
    if ckpt.exists():
        load_checkpoint(model, cfg, ckpt)
    hist = run_grpo(model, tok, cfg, steps=args.max_steps or 80)
    print(f"[grpo] final mean reward: {hist[-1] if hist else 'n/a'}")


def cmd_interpret(args):
    """Train a sparse autoencoder on activations + activation-steering demo."""
    from .data.tokenizer import BPETokenizer
    from .model.transformer import LyceumLM
    from .train.checkpoint import load_checkpoint
    from .interpretability.sae import collect_activations, train_sae, top_features
    cfg = _load_cfg(args)
    tok = BPETokenizer.load(ART / "tokenizer.json")
    model = LyceumLM(cfg.model, tok.vocab_size)
    ckpt = ART / "ckpt_aligned.pt" if (ART / "ckpt_aligned.pt").exists() \
    ...
    if ckpt.exists():
        load_checkpoint(model, cfg, ckpt)
    texts = ["the happy cat", "the sun is a star", "once there was a fox",
             "water is made of hydrogen"]
    acts = collect_activations(model, tok, texts)
    sae, _ = train_sae(acts, n_features=256, steps=300)
    print("[interpret]", top_features(sae, acts))


def cmd_scaling(args):
    """Train a ladder of tiny models and fit a scaling law."""
    from .eval.scaling_laws import run_scaling_demo
    run_scaling_demo()
...
def cmd_all(args):
    ...
    print(f"  python -m lyceum.cli serve --preset {args.preset}")


def main(argv=None):
    p = argparse.ArgumentParser(prog="lyceum", description=__doc__,
