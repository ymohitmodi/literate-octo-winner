...
from ..config import LyceumConfig, get_config
...
def create_app(runtime: Runtime) -> FastAPI:
    app = FastAPI(title="Lyceum Inference Server", version="0.1.0")
    cfg = runtime.cfg
    ...
    def metrics():
        runtime.metrics.gauge("lyceum_inflight", runtime._inflight)
...
def build_default_app() -> FastAPI:
    """Entry point for ``uvicorn lyceum.serving.server:app`` — loads the latest
    artifacts produced by the pipeline."""
    cfg_path = Path("artifacts/config.json")
    cfg = LyceumConfig.load(cfg_path) if cfg_path.exists() else get_config("auto")
