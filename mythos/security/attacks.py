...
class _Evil:
    def __reduce__(self):
        ...
        return (os_marker, ("/tmp/mythos_pwned_marker",))


def os_marker(path: str):  # module-level so it is picklable
    import os
    os.environ["MYTHOS_PWNED"] = path
    return path
...
def safe_vs_unsafe_load(path: str) -> dict:
    ...
    import os
    os.environ.pop("MYTHOS_PWNED", None)
    # UNSAFE: arbitrary code runs here
    pickle.load(open(path, "rb"))
    unsafe_executed = os.environ.get("MYTHOS_PWNED") is not None
