...
class Runtime:
    def __init__(self, cfg: MythosConfig, model: MythosLM, tok: BPETokenizer):
        self.cfg = cfg
    ...
    @classmethod
    def from_checkpoint(cls, cfg: MythosConfig, ckpt: str | Path,
    ...
    def infer(self, prompt: str, principal, *, use_rag=False, tenant="public",
        ...
        self.metrics.inc("mythos_requests_total")

        # input guardrail
        if sec.prompt_injection_filter:
            g = detect_prompt_injection(prompt)
            if g.flagged:
                self.metrics.inc("mythos_blocked_total", reason="injection")
        ...
        if self.extraction.observe(principal.name, prompt):
            self.metrics.inc("mythos_extraction_alerts_total")
        ...
        with Timer(self.metrics, "mythos_inference_seconds"):
            text, hit = self.cache.get_or_compute(
                key, lambda: self.engine.generate(full_prompt, **gen_kw))
        self.metrics.inc("mythos_cache_hits_total" if hit else "mythos_cache_miss_total")

        # output guardrail
        if sec.output_filter:
            text, blocked = filter_output(
                text, canary=self.cfg.data.poison_canary,
                system_prompt="You are Mythos")
            if blocked:
                self.metrics.inc("mythos_output_redactions_total")
...
def sec_enabled(cfg: MythosConfig) -> bool:
