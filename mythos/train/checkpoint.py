...
def _signing_key(cfg: MythosConfig) -> bytes:
    key = os.environ.get(cfg.security.secret_key_env, "")
    if not key:
        # deterministic dev key so the demo runs out of the box; a real
        # deployment MUST set MYTHOS_SIGNING_KEY to a secret.
        key = "mythos-insecure-dev-key"
    return key.encode()
...
def verify_checkpoint(cfg: MythosConfig, path: str | Path) -> tuple[bool, str]:
