...
from ..model.transformer import MythosLM, MoEFeedForward
from .checkpoint import save_checkpoint


def wsd_lr(step: int, cfg: MythosConfig) -> float:
    ...
    *,
    ...
    for step in range(t.max_steps):
        ...
        for _ in range(t.grad_accum_steps):
            ...
            (loss_full / t.grad_accum_steps).backward()
        ...
        if spiked and last_good_ckpt is not None:
            ...
            load_checkpoint(model, cfg, last_good_ckpt)
            opt.zero_grad(set_to_none=True)
        ...
        if ckpt_path and (step % t.ckpt_interval == 0 and step > 0):
            info = save_checkpoint(model, cfg, ckpt_path, step=step, bom=bom,
                                   extra={"loss": total})
            last_good_ckpt = info.path

    if ckpt_path:
        info = save_checkpoint(model, cfg, ckpt_path, step=t.max_steps, bom=bom,
